...
def bSTR(instr):
    ...
    return chr(len(instr)) + "\x82" + instr

def bERROR(instr):
    assert len(instr) < 128
    return chr(len(instr)) + "\x8d" + instr
...
# PY3KPORT BEGIN - Additional test functions for testing mostly
# byte strings occuring in dictionary keys, values and in lists.
def key_to_bytes(indict, encoding='latin1'):
    """ Return a dictionary where keys have been converted from str->byte """

    indict2 = {}
    for k,v in indict.items():
        try:
            indict2[six.ensure_binary(k, encoding)] = v
        except Exception as e:
            indict2[k] = v          

    return indict2

def key_to_bytes_list(inlist, encoding='latin1'):
    """ Return a dictionary where keys have been converted from str->byte """

    inlist2 = []

    for indict in inlist:
        inlist2.append(key_to_bytes(indict, encoding))

    return inlist2

def value_to_bytes(indict, encoding='latin1'):
    """ Return a dictionary where values have been converted from str->byte """

    indict2 = {}
    for k,v in indict.items():
        if type(v) is not list:
            try:
                indict2[k] = six.ensure_binary(v, encoding)
            except Exception as e:
                indict2[k] = v
        else:
            v2 = []
            for item in v:
                try:
                    v2.append(six.ensure_binary(item, encoding))
                except Exception as e:
                    v2.append(item)
...
def list_to_bytes(inlist, encoding='latin1'):
    """ Map each item to bytes wherever possible """

    inlist2 = []
    for item in inlist:
        try:
            inlist2.append(six.ensure_binary(item, encoding))
        except Exception as e:
            inlist2.append(item)

    return inlist2

# PY3KPORT - End additional test functions
    ...
    for t in tokens:
        if isinstance(t, tuple):
            ...
            if isinstance(t, int):
                ...
                data.append(STRING)
                # PY3KPORT - Ensure bytes
                data.append(six.ensure_binary(t))
            else:
...
class TestBananaMixin:
    ...
    def makeBanana(self):
        ...
        self.banana.slicerClass = storage.UnsafeStorageRootSlicer
        self.banana.unslicerClass = storage.UnsafeStorageRootUnslicer
        self.banana.transport = TestBytesTransport()
        self.banana.connectionMade()
    ...
    def wantEqual(self, got, wanted):
        got_b = six.ensure_binary(got, 'latin1')
        wanted_b = six.ensure_binary(wanted, 'latin1')
    ...
    def looptest(self, obj, newvalue=_None):
        ...
        if type(obj) in (str, bytes, dict):
            # PY3KPORT: Cuz the values differ now
            # due to str->byte conversion.
            d.addCallback(self._looptest_2, newvalue)
        else:
    ...
    def _looptest_2(self, obj2, newvalue):

        # PY3KPORT: Separate callback function
        # for dict and str/bytes. No type
        # checking is done here.
        if type(obj2) is dict:
            self.assertEqual(key_to_bytes(obj2),
                             key_to_bytes(newvalue))
        else:
            self.assertEqual(six.ensure_binary(obj2, 'latin1'),
                             six.ensure_binary(newvalue, 'latin1'))
...
class BrokenDictUnslicer(DictUnslicer):
    ...
    def receiveKey(self, key):
        ...
        if key in ("die", six.b('die')):
            raise Violation("aaagh")
        if key in ("please_die_in_finish",
                   six.b("please_die_in_finish")):
                   
            self.dieInFinish = 1
        DictUnslicer.receiveKey(self, key)

    def receiveValue(self, value):
        if value in ("die", six.b("die")):
...
class DecodeTest(UnbananaTestMixin, unittest.TestCase):
    def setUp(self):
        ...
        d ={ ('dict1',): BrokenDictUnslicer,
             ...
             (six.b('dict1'),): BrokenDictUnslicer,
             (six.b('dict2'),): ReallyBrokenDictUnslicer             
             }
    ...
    def test_simple_list(self):
        ...
        self.assertEqual(res, [1,2,3, six.b('a'),six.b('b')])
    ...
    def test_dict(self):
        ...
        # PY3KPORT
        self.assertEqual(res, key_to_bytes({'a':1, 'b':2}))
    ...
    def test_dict_with_duplicate_keys(self):
        ...
        if six.PY2:
            ...
            self.assertEqual(f.value.args[0], "duplicate key 'b'a''")
    ...
    def test_dict_with_list(self):
        ...
        self.assertEqual(res, key_to_bytes({'a':1, 'b':[2,3]}))
    ...
    def test_dict_with_tuple_as_key(self):
        ...
        self.assertEqual(res, value_to_bytes({(1,2): 'a'}))
    ...
    def test_instance_bad1(self):
        ...
        if six.PY2:
            ...
            self.assertEqual(f.value.where, "<RootUnslicer>.<b'Foo'>.b'c'.<??>")
    ...
    def test_instance_bad2(self):
        ...
        if six.PY2:
            ...
            self.assertEqual(f.value.where,
                            "<RootUnslicer>.<b'Foo'>.b'c'.<b'Bar'>.attrname??")
    ...
    def test_instance_unsafe1(self):
        ...
        if six.PY2:
            self.assertEqual(f.value.args[0], "unknown top-level OPEN type ('instance',)")
        elif six.PY3:
    ...
    def test_ref4(self):
        ...
        self.assertEqual(res, key_to_bytes_list(wanted))
        self.failUnlessIdentical(res[0], res[1])
    ...
    def test_failed_dict2(self):
        # dies during start
        xyz = self.shouldFail([tOPEN(0),'list', 1,
                               tOPEN(1),'dict2', "a", 2, "b", 3, tCLOSE(1),
                               tCLOSE(0)])
        self.assertTrue(isinstance(xyz, BananaFailure))
        
        self.assertTrue(xyz.check(Violation))
        
        self.assertEqual(xyz.value.where, "<RootUnslicer>.[1].{}")
        self.assertEqual(xyz.value.args[0], "dead in start")
    ...
    def test_failed_dict4(self):
        ...
        if six.PY2:
            ...
            self.assertEqual(f.value.where, "<RootUnslicer>.[1].{}[b'b']")
...
class Bar:
    ...
    def compare(self, them):
        ...
        if six.PY2:
            ...
            # PY3KPORT
            # Can have bytes as keys from strings
            self_bdict = key_to_bytes(self.__dict__)
            them_bdict = key_to_bytes(them.__dict__)

            val = (self.__class__, self_bdict) == (them.__class__, them_bdict) or \
                  (self.__class__, self.__dict__) == (them.__class__, them.__dict__)

            return val

# PY3KPORT
if six.PY2:
    Bar.__cmp__ = Bar.compare
elif six.PY3:
    Bar.__eq__ = Bar.compare
...
class EncodeFailureTest(unittest.TestCase):
    ...
    def _test4_1(self, e, s):
        ...
        # PY3KPORT: Are these kind of tests useful ?
        if six.PY2:
            self.failUnlessSubstring("cannot serialize <open file",
                                     e.value.args[0])
        elif six.PY3:
            self.failUnlessSubstring("cannot serialize <_io.TextIOWrapper",
                                     e.value.args[0])
    ...
    def _test_instance_unsafe_1(self, e):
        ...
        # PY3KPORT: In Py3 its "object" and Py2 "instance" so no need
        # to go checking to that extreme!
        self.assertTrue(why.startswith("cannot serialize <foolscap.test.test_banana.Foo"))
        # Well - not sure why we need tests like this but here goes anyway :-|
        if six.PY2:
...
class DecodeFailureTest(TestBananaMixin, unittest.TestCase):
    ...
    def setUp(self):
        ...
        d = {('errorful',): ErrorfulUnslicer,
             ...
             (six.b('errorful'),): ErrorfulUnslicer,
             (six.b('failing'),): FailingUnslicer          
             }
        self.banana.rootUnslicer.topRegistries.insert(0, d)
        self.banana.rootUnslicer.openRegistries.insert(0, d)
    ...
    def testSuccess1(self):
        ...
        self.assertEqual(o, (six.b('a'),[1,[2,3]], six.b('b')))
...
class InboundByteStream(TestBananaMixin, unittest.TestCase):
    ...
    def check(self, obj, stream):
        ...
        if type(obj) is dict:
            obj = value_to_bytes(obj)
            obj2 = value_to_bytes(obj2)
        elif type(obj) is list:
            obj = list_to_bytes(obj)
            obj2 = list_to_bytes(obj2)
        elif type(obj) in (str, bytes):
            obj = six.ensure_binary(obj)
            obj2 = six.ensure_binary(obj2)
...
class InboundByteStream2(TestBananaMixin, unittest.TestCase):
    ...
    def conform2(self, stream, obj, constraint=None, childConstraint=None):
        ...
        if type(obj) in (str, bytes):
            self.assertEqual(six.ensure_binary(obj), six.ensure_binary(obj2))
        else:
            if type(obj) is dict:
                obj = value_to_bytes(obj)
                obj2 = value_to_bytes(obj2)
                ...
                obj = tuple(list_to_bytes(obj))
                obj2 = tuple(list_to_bytes(obj2))
...
# PY3KPORT: Inherit from Bar to get the __eq__ implementation for free
class A(Bar):
    """
    dummy class
    """
...
class ThereAndBackAgain(TestBananaMixin, unittest.TestCase):
    ...
    def _testStressReferences_1(self, z):
        self.assertIdentical(z[0][six.b('list')], z[1])
        self.assertIdentical(z[0][six.b('list')][0], z)
    ...
    def test_cycles_2(self):
        ...
        a['foo'] = t2
        d = self.loop(t1)
        d.addCallback(lambda z: self.assertIdentical(z[0][six.b('foo')][0], z))
        return d
...
class VocabTest1(unittest.TestCase):
    def test_incoming1(self):
        ...
        self.assertEqual(b.incomingVocabulary, value_to_bytes(vdict))
...
class VocabTest2(TestBananaMixin, unittest.TestCase):
    ...
    def test_loop(self):
        ...
        self.assertEqual(self.banana.incomingVocabulary, value_to_bytes(vdict))
        self.clearOutput()
